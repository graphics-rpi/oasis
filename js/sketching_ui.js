 ...
 **********************************************************************/
// console.log("Loading sketching_ui.js");
...
CURSOR_X = 0; // Mouse  X Location GLOBALS
CURSOR_Y = 0; // Mouse  Y Location GLOBALS

var DRAG_START = new Array(); // Point at which you begin drag
...
var table = null; // Invisable element that we click on
...
var task_form_id_array = new Array(); // A string of form ids for all task objects for easy access
...
function CANVAS_H()
{
  // return $('#container').outerHeight() - $('#container').offset().top;
...
function generate_tabletop()
{
  ...
  paper = Raphael('container', CANVAS_W(), CANVAS_H());
  ...
  var cx = CANVAS_W() / 2.0;
  var cy = CANVAS_H() / 2.0;
  var radius = Math.min(cx, cy) - 0.05 * Math.min(CANVAS_W(), CANVAS_H());
  ...
  .data("primitive", "table-background")
  .click(function(){
  ...
  table = paper.circle(cx, cy, radius)
    .attr(
    {
      'fill': "red",
      'fill-opacity': .0
...
function generate_compass()
{
  ...
  orientation = paper.rect(0, 0, CANVAS_W(), CANVAS_H())
    .attr(
    {
      'fill': "blue",
      'fill-opacity': .0,
...
function calc_long_lat(marker_abs_x, marker_abs_y,raphael_map)
{
  ...
  var mx = raphael_map.attr('width');
  var my = raphael_map.attr('height');
  ...
  paper.forEach(
    function(el)
    {
      ...
      if (el.data('primitive') == 'WALL_ST8')
      {
        ...
        var angle = Math.min(
          angleBtwVec(win_dir, wall_dir),
          angleBtwVec(negateVec(win_dir), wall_dir)) * (180 / Math.PI);
...
function sketching_ribbon_handler(event_str)
{
  ...
  if (event_str == "button-3d-fcv")
  {
    ...
    $.post(
      "../php/toggle_visualization_mode.php",
      {},
      function()
      {
        // Reload the page
...
function hideAllHandles(){
 ...
 for(var i = 0; i < freetransform_list.length; i++){
    freetransform_list[i].hideHandles({undrag: false});
  }
    ...
    function(ft, events)
    {
      ...
      if( events == "drag start"){
        ...
        if( SKETCH_MODE == "CURSOR_MODE"){
          ...
          if
            (
              table.isPointInside(uL[0], uL[1]) &&
              table.isPointInside(uR[0], uR[1]) &&
              table.isPointInside(lR[0], lR[1]) &&
              table.isPointInside(lL[0], lL[1]) 
            )
          {
...
function bind_furniture_handlers(raphael_obj, ITEM_STR)
{
  ...
  /* old click function for removal
      raphael_obj.click(function()
      {
      ...
      */
      ...
      /* old drag function
        // When users drag the item around, we want to make sure
        // that all the corners are on our table
        raphael_obj.drag(
          function()
          {
...
function create_skylight(xpos, ypos)
{
  ...
  var rect = paper.rect(xpos, ypos, WINDOW_WIDTH, WINDOW_HEIGHT)
    .attr('fill', 'blue' );
...
function create_bed(xpos, ypos)
{
  ...
  var rect = paper.image('../images/bed.jpg',xpos, ypos, BED_WIDTH, BED_HEIGHT)
    .attr('fill', 'green' );
...
function create_desk(xpos,ypos)
{
  ...
  var rect = paper.image('../images/desk.png',xpos,ypos, DESK_WIDTH, DESK_HEIGHT)
      .attr('fill', 'orange');
...
function create_closest(xpos,ypos)
{
  ...
  var rect = paper.image('../images/closet.png',0,0, WARDROBE_WIDTH, WARDROBE_HEIGHT)
      .attr('fill', 'brown');
...
function table_start_drag()
{
  ...
  else if (SKETCH_MODE == "CURSOR_MODE")
  {

  }
  else if (SKETCH_MODE == "COMPASS_MODE")
  {
...
function table_end_drag()
{
  ...
  else if (SKETCH_MODE == "WIN_ST8_MODE")
  {
    ...
    var length = Math.sqrt(
      Math.pow(CURSOR_X - DRAG_START[0], 2) +
      Math.pow(CURSOR_Y - DRAG_START[1], 2)
    );
      ...
      $.post(
        ...
        function()
        {
          if (run_remesh && (hasWindows() || modelHasWindows()) )
          {
            ...
            {
              // Call this function when task_remesh is done
...
/*function load_non_submitted_task()
{
  ...
  {
    type: "POST",
    url: "../php/get_non_submitted_task.php",
    async: false,
    success: function(e)
    {
      task_container = JSON.parse(JSON.parse(e).data);
    }
  });

  if (task_container == "")
  {
    // alert("No non submitted task");
    return;
  }

  var task_obj_array = task_container.task_obj_array

  // For each task object saved there you want to load it up


  // Given a task obj we will prepend all of these new objects
  for (var ii = 0; ii < task_obj_array.length; ii++)
  {
    var taskobj = task_obj_array[ii];
    // ==========================================================================
    // Form
    // ==========================================================================
    var task_id = "new_task_form_" + new_task_count;
    ...
    date.setAttribute('type', "date");
    date.setAttribute('name', "date");
    date.setAttribute('value', taskobj.date);
    ...
    time.setAttribute('type', "time");
    time.setAttribute('name', "time");
    time.setAttribute('value', taskobj.time);
    ...
    var timezone = document.createElement('select');
    timezone.setAttribute('name', "timezone");
    createOption(timezone, "(GMT -12:00) Eniwetok, Kwajalein", "-12:00");
    createOption(timezone, "(GMT -11:00) Midway Island, Samoa", "-11:00");
    createOption(timezone, "(GMT -10:00) Hawaii", "-10:00");
    createOption(timezone, "(GMT -9:00) Alaska", "-9:00");
    createOption(timezone, "(GMT -8:00) Pacific Time US, Canada", "-8:00");
    createOption(timezone, "(GMT -7:00) Mountain Time US, Canada", "-7:00");
    createOption(timezone, "(GMT -6:00) Central Time US, Canada, Mexico City", "-6:00");
    createOption(timezone, "(GMT -5:00) Eastern Time US, Canada, Bogota", "-5:00", true);
    createOption(timezone, "(GMT -4:00) Atlantic Time Canada, Caracas, La Paz", "-4:00");
    createOption(timezone, "(GMT -3:30) Newfoundland", "-3.30");
    createOption(timezone, "(GMT -3:00) Brazil, Buenos Aires, Georgetown", "-3:00");
    createOption(timezone, "(GMT -2:00) Mid-Atlantic", "-2:00");
    createOption(timezone, "(GMT -1:00 hour) Azores, Cape Verde Islands", "-1:00");
    createOption(timezone, "(GMT) Western Europe Time, London, Lisbon", "0:00");
    createOption(timezone, "(GMT +1:00) Brussels, Copenhagen, Madrid, Paris", "1:00");
    createOption(timezone, "(GMT +2:00) Kaliningrad, South Africa", "2:00");
    createOption(timezone, "(GMT +3:00) Baghdad, Riyadh, Moscow", "3:00");
    createOption(timezone, "(GMT +3:30) Tehran", "3:30");
    createOption(timezone, "(GMT +4:00) Abu Dhabi, Muscat, Baku, Tbilisi", "4:00");
    createOption(timezone, "(GMT +4:30) Kabul", "4:30");
    createOption(timezone, "(GMT +5:00) Ekaterinburg, Islamabad, Karachi", "5:00");
    createOption(timezone, "(GMT +5:30) Bombay, Calcutta, Madras, New Delhi", "5:30");
    createOption(timezone, "(GMT +5:45) Kathmandu", "5:45");
    createOption(timezone, "(GMT +6:00) Almaty, Dhaka, Colombo", "6:00");
    createOption(timezone, "(GMT +7:00) Bangkok, Hanoi, Jakarta", "7:00");
    createOption(timezone, "(GMT +8:00) Beijing, Singapore, Hong Kong", "8:00");
    createOption(timezone, "(GMT +9:00) Tokyo, Seoul, Osaka, Sapporo", "9:00");
    createOption(timezone, "(GMT +9:30) Adelaide, Darwin", "9:30");
    createOption(timezone, "(GMT +10:00) Eastern Australia, Guam", "10:00");
    createOption(timezone, "(GMT +11:00) Magadan, Solomon Islands", "11:00");
    createOption(timezone, "(GMT +12:00) Auckland, Wellington, Fiji", "12:00");
...
//  alert(timezone_hour);
  ...
  //console.log(mn, dd);
  //console.log(hh, mi);
  //console.log(timezone_sign, timezone_hour, timezone_min);
  //console.log(w);
...
/*function save_non_submitted_task(task_id)
{
  // If this was called from clicking a button (not refresh or change tab)
  // We want to remove this task_id from being saved
  if (task_id != 'None')
  {
    ...
    {
      if (task_form_id_array[i] == task_id)
      {
        index = i;
        break;
      }
    ...
    if (index == -1)
    {
      alert("task_form_id_array(task_id): invalid task_id " + task_id);
      window.location = '../pages/login_page.php';
    }
    else
    {
      task_form_id_array.splice(index, 1);
    }
  }

  // this function will generate the task array for you
  var task_obj_array = generate_task_array();

  // Object container that holds our task
  var task_container = {
    task_obj_array: task_obj_array
  };

  // alert("making call to save_non_submitted_task");

  $.ajax(
  {
    type: "POST",
    url: "../php/save_non_submitted_task.php",

    data:
    {
      json: JSON.stringify(task_container)
    },
    ...
    success: function()
    {
      // console.log("Return from save_non_submitted_task");
    }
...
function load_error(id, path)
{
  ...
  error: function(e){
    ...
    setTimeout(function(){ ready_ajax("Ready"); }, 3000);
    // console.log("Failed to determine if developer");
  }
});
}
